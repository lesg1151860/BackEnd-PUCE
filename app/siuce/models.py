...
class TipoAgresion(models.Model):
    ...
    def __str__(self): return self.nombre # <-- Esto soluciona lo del ID
...
class ClasificacionLey1620(models.Model):
    ...
    def __str__(self): return f"{self.tipo}: {self.descripcion[:30]}..." # <-- Muestra Tipo y descripción
...
class CasoSIUCE(models.Model):
    ...
    """
    # Referencia al SAC (Es un CharField porque son bases de datos independientes)
    ...
    # Campos calculados (se guardan para historial, pero son controlados por el save())
