...
from .views import CambiarPasswordView, MyTokenObtainPairView, RegistroUsuarioView, UsuarioDetailView, UsuarioListCreateView

urlpatterns = [
    path('login/', MyTokenObtainPairView.as_view(), name='token_obtain_pair'), # Ruta para obtener el token JWT
    path('login/refresh/', MyTokenObtainPairView.as_view(), name='token_refresh'), # Ruta para refrescar el token JWT
    ...
    path('usuarios/<int:pk>/cambiar-contrasena/', CambiarPasswordView.as_view(), name='cambiar_contrasena'),
]
